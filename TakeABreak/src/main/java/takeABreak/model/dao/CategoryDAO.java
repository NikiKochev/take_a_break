...
public class CategoryDAO {
    ...
    public List<Post> getPostByCategory(List<String> text, int page, int perpage) {
        StringBuilder query = new StringBuilder("SELECT posts.id FROM posts JOIN categories ON (categories_id = categories.id) ");
    ...
    private StringBuilder add(StringBuilder query, String value) {
        if(query.compareTo(new StringBuilder("SELECT posts.id FROM posts JOIN categories ON (categories_id = categories.id) ")) != 0){
            query.append(" AND ");
        }
        else {
            query.append("WHERE ");
